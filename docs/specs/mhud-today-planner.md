...
**Review fixes (frontend; 27 findings from 4 lenses, 5 confirmed by both refuters, the a11y set adopted outright)**
- A fresh `Composer` per item (`key={item.id}`); `flush()` waits for an in-flight save and retries a failed one, and generate/handoffs refuse to proceed while the server lacks the on-screen text; a generate that lands after a draft switch never writes into the wrong editor; undo disappears on the next edit; new-draft failures are shown.
- `approve & send` has an in-flight guard; the Google upgrade link only accepts an app-relative path; the markdown map renders images as their alt text; `?date=` is validated and a failed revalidation keeps the loaded page with a notice; `usePlanner` applies and rolls back only the acted row and `plan()` / `refresh()` never reject.
- Accessibility: the list keeps its focus ring and exposes `aria-activedescendant`; row titles are buttons (keyboard-selectable on resolved rows too); the snooze menu takes focus on open, cycles with the arrow keys and closes on Escape from anywhere; handoff inputs carry placeholders; save status is a polite live region with the err token on failure; long titles wrap in the workspace.

**Sandbox notes.** This box has no `sqlite3` CLI (the two `__tests__/prisma` migration suites are env-red here, green in CI) and ships a different Chromium build than `@playwright/test` pins; the e2e run used an untracked Playwright config override with `launchOptions.executablePath` and a `sqlite3` shim over `prisma db execute`. Final local numbers on the reviewed tree: 1,707 unit/integration tests green (7 env-red), 22/22 Playwright, `tsc` and ESLint clean, `next build` green; CI green on the same head.
