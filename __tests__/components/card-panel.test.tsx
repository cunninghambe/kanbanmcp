...
const orgMembers = [
  { userId: 'user-1', user: { id: 'user-1', name: 'Alice', email: 'alice@example.com', isAgent: false } },
  { userId: 'user-2', user: { id: 'user-2', name: 'Bob', email: 'bob@example.com', isAgent: false } },
]
...
function setupMocks({ currentUserId = 'user-99', card = baseCard } = {}) {
  ...
  mockUseSWR.mockImplementation((key) => {
    ...
    return {
      data: {
        user: { id: currentUserId, name: 'Test User', email: 'test@example.com' },
...
describe('CardModal', () => {
  ...
  it('does NOT show SignoffPanel when current user is not reviewer or approver', () => {
    ...
    expect(screen.queryByTestId('signoff-panel-REVIEWER')).not.toBeInTheDocument()
    expect(screen.queryByTestId('signoff-panel-APPROVER')).not.toBeInTheDocument()
  })
  ...
  it('shows parent breadcrumb when card has parentCardId', () => {
    const cardWithParent = {
      ...
      parent: { id: 'parent-card-1', title: 'Parent Epic', aiReviewParams: null },
    }
