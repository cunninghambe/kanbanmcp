...
function makeNode(
  overrides: Partial<SubtreeNode> & { id: string; title: string },
): SubtreeNode {
  return {
    id: overrides.id,
...
function setupMock(
  data:
    | { root: SubtreeNode; descendants: SubtreeNode[]; truncated: boolean }
    | undefined,
...
describe("SubcardTree", () => {
  ...
  it("renders Sub-cards section heading", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 3, name: /Sub-cards/i }),
    ).toBeInTheDocument();
  });
});
