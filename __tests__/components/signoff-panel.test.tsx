...
const latestReviewerSignoff: ExistingSignoff = {
  ...
  user: { id: 'user-reviewer', name: 'Alice Reviewer', email: 'alice@example.com' },
}
...
describe('SignoffPanel', () => {
  ...
  it('renders approve, request changes, and reject buttons', () => {
    ...
    expect(screen.getByRole('button', { name: /Request changes to this card/i })).toBeInTheDocument()
  ...
  it('posts to signoffs API with correct payload on Approve click', async () => {
    ...
    const approveBtn = screen.getByRole('button', { name: /Approve this card/i })
  ...
  it('shows error message when API call fails', async () => {
    ...
    vi.mocked(global.fetch).mockResolvedValueOnce({
      ok: false,
      status: 403,
      json: async () => ({ error: 'Only the assigned reviewer may sign off as REVIEWER' }),
    } as Response)
