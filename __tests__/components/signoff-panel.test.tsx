...
describe("SignoffPanel", () => {
  ...
  it("renders approve, request changes, and reject buttons", () => {
    render(
      <SignoffPanel cardId="card-1" role="REVIEWER" onSubmitted={vi.fn()} />,
    );

    expect(
      screen.getByRole("button", { name: /Approve this card/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Request changes to this card/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Reject this card/i }),
    ).toBeInTheDocument();
  });
  ...
  it("disables buttons while submitting", async () => {
    ...
    render(
      <SignoffPanel cardId="card-1" role="APPROVER" onSubmitted={vi.fn()} />,
    );
  ...
  it("shows success message after successful submission", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("status")).toHaveTextContent(
        /Approved successfully recorded/i,
      );
    });
  });
