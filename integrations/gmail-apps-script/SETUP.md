...
6. Deploy → New deployment → **Web app**, execute as _Me_, access
   _Anyone_. Copy the `/exec` URL — that's `INBOX_AGENT_URL` below. Auth on
...
The deployed `/exec` endpoint answers four actions, all authenticated by
`WEBHOOK_TOKEN` in the JSON body and all called server-side only:

| `action`  | Body                                            | Used by                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `draft`   | `{ threadId, instructions, replyAll? }`         | the card reply panel — the script asks the model to write the reply                                                                                                                                                                                                                                                                                                                                                |
| `compose` | `{ body, threadId?, replyAll?, to?, subject? }` | the Today planner's email handoff — **verbatim body in, Gmail draft out; the script never calls the model**. With `threadId` it drafts a reply to the last message on that thread; otherwise `to` + `subject` start a new message. Returns `{ draftId, preview, to, cc }`, and those recipients are what the planner shows before you approve the send. Rejects an empty body and anything over 20 000 characters. |
| `send`    | `{ draftId }`                                   | both flows, once you approve the preview                                                                                                                                                                                                                                                                                                                                                                           |
| `ack`     | `{ threadId }`                                  | the nudge ack — clears the `ai/urgent` label                                                                                                                                                                                                                                                                                                                                                                       |

`compose` needs no new Script Properties: it reuses the same
`WEBHOOK_TOKEN`, and mhud reaches it through the same `INBOX_AGENT_URL` /
`INBOX_AGENT_TOKEN` pair. The planner is additionally gated by
`INBOX_AGENT_OWNER` on the mhud side (the mailbox belongs to one person,
so only that person's session may compose or send), and it sends only the
draft id the server itself received from `compose` — a draft id coming
from the browser is never relayed.
