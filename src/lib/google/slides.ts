...
async function fetchImageBase64(contentUrl: string, token: string): Promise<string | null> {
  const res = await googleFetch(contentUrl, { headers: { Authorization: `Bearer ${token}` } })
...
async function fetchSlideImages(
  ...
  const results = await Promise.all(capped.map((url) => fetchImageBase64(url, token)))
...
export async function extractSlides(userId: string, fileId: string): Promise<SlideContent[]> {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    const { dataUrls, skipped } = await fetchSlideImages(imageUrls, token)
